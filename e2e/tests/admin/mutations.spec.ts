...
test.describe('@p0 admin mutations', () => {
  test('TC-AD05 deactivates and restores a customer and login follows status', async ({
    adminApi,
    guestApi,
    uniqueData,
  }) => {
  ...
  test('TC-AD08 deactivates and restores staff and login follows status', async ({
    adminApi,
    guestApi,
    uniqueData,
  }) => {
  ...
  test('TC-AD11 deactivates and restores a service', async ({ adminApi, uniqueData }) => {
  ...
  test('TC-AD12 creates, updates, and deactivates a scoped promotion', async ({ adminApi, uniqueData }) => {
    const services = await (await adminApi.get('/api/services')).json() as {
      data: Array<{ serviceId: number; active: boolean }>;
    };
