...
test.describe.serial('@p0 customer account and payment', () => {
  ...
  test.skip('TC-CU06 exposes the pending billing ID and creates VNPay for that exact billing', async ({
    customerApi,
  }) => {
  ...
  test.skip('TC-CU07 cancels a pending booking without creating a refund voucher', async ({ customerApi }) => {
  ...
  test.skip('TC-CU15 applies promotion, percentage voucher, and 30 percent deposit consistently', async ({
    customerApi,
  }) => {
    const rewards = await activeRewards(customerApi);
